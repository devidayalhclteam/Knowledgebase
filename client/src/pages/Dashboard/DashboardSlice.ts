...
const blobSasUrl =
  "https://devgurukulstorage.blob.core.windows.net/?sv=2021-06-08&ss=bfqt&srt=sco&sp=rwdlacupiytfx&se=2023-03-06T11:23:49Z&st=2023-02-06T03:23:49Z&spr=https,http&sig=1rHz3PHMMb3Lwd8gTB7gcZQBGKrU57u%2FlB2uBqh3zPc%3D";
...
export const deleteProduct = createAsyncThunk("dashboard/deleteProduct", async (payload: any) => {
...
export const deleteProductImages = createAsyncThunk("dashboard/deleteProductImages", async (payload: any) => {
  console.log("delet payload", payload);
  try {
...
export const uploadImage = createAsyncThunk("dashboard/uploaImage", async (payload: any) => {
  if (payload?.length) {
    try {
...
export const deleteImage = createAsyncThunk("dashboard/deleteImage", async (payload: any) => {
  if (payload?.length) {
    try {
...
const productsSlice = createSlice({
  ...
  reducers: {
    setProductFormData: (
      state: any,
      action: PayloadAction<React.ChangeEvent<HTMLInputElement> | SelectChangeEvent>
    ) => {
