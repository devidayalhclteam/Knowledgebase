...
const productListSelector = createSelector(productImagesState, dashboardSelect, productListSelect, categoriesSelect,
    (state: any, dashboardState: any, productListState: any, categoriesState: any) => {
        ...
        if (searchInput && !searchSelect) {
            searchedProducts = products.filter((product: Prod) =>
            ...
            searchedProducts = products.filter((product: Prod) =>
                searchSelect === product.categoryId && product.productName.toLowerCase().indexOf(searchInput.trim().toLowerCase()) >= 0
            );
        } else {
        ...
        if (categoriesState.categoryResponse.data) {
            searchSelectName = categoriesState.categoryResponse.data.find((category: any) => category.id === searchSelect)?.name;
        }
        ...
        if (productListState.sortOrder === "asc") {
            ...
            searchedProducts.sort((productA, productB) =>
                productA.productName.toLowerCase().localeCompare(productB.productName.toLowerCase())).reverse();
        }
